...
def _safe_extract_products(cas_number: str, result: SearchResult, supplier: str):
    ...
    try:
        ...
        return [ExtractedProductData(
            ...
            confidence=0,
            evidence=f"v16 guarded extraction failure; source preserved for manual review: {str(exc)[:300]}",
...
def summarize_supplier_rows(detail_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for supplier, g in df.groupby("supplier", dropna=False):
        ...
        row = {
            ...
            "notes": "v16 grouped row. CAS-confirmed public prices drive the model. CAS-unconfirmed public prices are retained as lower-confidence evidence only.",
            "data_source": "live_supplier_adapter_summary_v16",
        }
        records.append(row)
...
def build_supplier_coverage_report(discovery_df: pd.DataFrame, detail_df: pd.DataFrame, max_suppliers: int, max_pages_to_extract: int) -> pd.DataFrame:
    ...
    for idx, adapter in enumerate(ADAPTERS, start=1):
        ...
        records.append({
            ...
            "notes": "v16 coverage: CAS-confirmed public prices are model-eligible; CAS-unconfirmed public prices are shown as lower-confidence evidence; fetch failures usually mean site blocking, wrong search endpoint, region/location selection, session, or JS constraints.",
        })
...
def _append_extracted_row(extracted_rows: list[dict], extracted, result: SearchResult, supplier: str, adapter, cas_number: str, chemical_name: str | None, snippet_price: float | None, price_visibility_status: str) -> None:
    extracted_rows.append({
        ...
        "data_source": "live_extraction_v16_identity_locked_supplier_parsers",
    })
...
def discover_live_suppliers(
    ...
    """v16 strict supplier-registry sourcing with supplier-specific parser coverage.
    ...
    extracted_rows = []
    total_extracted = 0
    # v16: as soon as any supplier confirms the CAS/product name, use that name to
    ...
    if chemical_name and chemical_name.strip() and chemical_name.strip().lower() not in {"acetaminophen", "optional"}:
        product_name_pool.add(chemical_name.strip())
    ...
    for supplier in supplier_order:
        ...
        for seed in seeds[:3]:
            try:
                ...
                continue
        # v16 supplier-specific product-name probes. These are especially important for
        ...
        candidates = sorted(candidates, key=lambda r: (0 if (r.source.startswith("expanded") or r.source.startswith("v16_product_name_probe") or r.source.startswith("v16_deferred_product_name_probe")) else 1, canonicalize_url(r.url)))
        ...
        for result in candidates:
            ...
            for extracted in extracted_products:
                ...
                # v16 keep rule: keep CAS-confirmed pages, meaningful supplier availability state,
    ...
    # v16 late product-name probe pass: revisit earlier suppliers after another source
        ...
        for supplier in supplier_order:
            ...
            for probe in probes[:2]:
                ...
                if candidate_key in processed_candidate_keys:
                    continue
                processed_candidate_keys.add(candidate_key)
                extra_discovery_records.append({"supplier": supplier, "title": probe.title, "url": probe.url, "canonical_url": canonicalize_url(probe.url), "domain": _domain(probe.url), "snippet": probe.snippet + " Late v16 confirmed-name probe.", "source": "v16_late_confirmed_name_probe", "supplier_hint": supplier})
    ...
    # v16: older deferred-probe block removed; late confirmed-name probes above now handle this
    ...
    if not detail_df.empty:
        ...
        # v16: keep CAS-unconfirmed public price rows as lower-confidence evidence; procurement_trust prevents them from entering the quantity model.
