        ...
        try:
            if "ambeed.com" in host:
                ...
                try:
                    ...
                    pass
            response = session.get(url, headers=hdrs, cookies=cookies, timeout=timeout, allow_redirects=True)
...
def _cas_identity_confidence(
    ...
    the visible pack/price rows belong to another compound. v16 therefore rejects a
    ...
    title = title or ""
    head = (text or "")[:90000]
    # v16.1: separate the true product identity/specification region from
    ...
    if trusted_discovery_cas and product_like:
        # v16.1: do not let generic CAS-like strings from related products/scripts undo
...
def _build_variant_product(base: ExtractedProductData, row: dict[str, Any]) -> ExtractedProductData:
    ...
    if base.cas_exact_match:
        ...
        evidence = base.evidence + f"; v16 quantity-price pair parsed from same supplier/generic structured row ({method})"
        ...
        evidence = base.evidence + f"; v16 lower-confidence public price candidate parsed from supplier row ({method}) but requested CAS was not confirmed in product identity"
        best_action = "Open source and verify CAS before using price"
    if product_form in {"solution", "standard/reference"}:
        evidence += f"; v16 product-form guard: {product_form}"
...
def extract_product_rows_from_url(cas_number: str, url: str, timeout: int = 18, supplier_hint: str | None = None, discovery_title: str | None = None, discovery_snippet: str | None = None) -> list[ExtractedProductData]:
    ...
    try:
        ...
        return [ExtractedProductData(
            ...
            confidence=10,
            evidence=f"Page could not be fetched or parsed after v16 browser/USA-location retry. Use source link for manual review. Error: {str(exc)[:260]}",
    ...
    rows: list[dict[str, Any]] = []
    # v16: supplier-specific parser profiles run first. Generic extractors remain as fallbacks.
    ...
    if not base.cas_exact_match:
        ...
        # v16 correction: search/result pages are navigation only. Do not parse their
