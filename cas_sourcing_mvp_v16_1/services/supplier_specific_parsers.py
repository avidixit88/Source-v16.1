...
def _parse_adjacent_text_pairs(profile: SupplierParserProfile, text: str) -> list[dict[str, Any]]:
    ...
    pack_price = re.compile(
        ...
        r'(?P<middle>.{0,140}?)'
        r'(?P<price>(?:US\$|\$)\s*[0-9][0-9,]*(?:\.\d{1,2})?|[0-9][0-9,]*(?:\.\d{1,2})?\s*USD)',
        re.I,
    )
...
def extract_supplier_specific_rows(supplier: str | None, soup: BeautifulSoup, text: str, url: str='') -> tuple[list[dict[str, Any]], str, str]:
    ...
    try:
        rows=[]; rows.extend(_parse_html_tables(profile,soup)); rows.extend(_parse_options(profile,soup)); rows.extend(_parse_json(profile,soup)); rows.extend(_parse_js_regex_pairs(profile,soup)); rows.extend(_parse_row_containers(profile,soup)); rows.extend(_parse_text_ladders(profile,text)); rows.extend(_parse_adjacent_text_pairs(profile,text)); rows=_dedupe(rows)
