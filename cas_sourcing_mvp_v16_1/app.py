...
st.set_page_config(
    page_title="CAS Sourcing MVP v16.1",
    page_icon="🧪",
    layout="wide",
)

st.title("🧪 CAS Sourcing & Procurement Intelligence MVP v16.1")
st.caption(
    "CAS input → supplier discovery → trusted catalog evidence → desired-quantity model → RFQ-ready shortlist. "
    "v16.1 fixes the runtime quote import issue, keeps search pages navigation-only, and uses balanced CAS identity gating so generic related-product CAS noise does not block valid supplier/product pages."
)
...
def render_price_and_bulk_sections(ranked: pd.DataFrame, desired_qty_g: float, desired_quantity: float, desired_unit: str) -> None:
    ...
    # v16: first show every extracted price lead, including CAS-unconfirmed candidates.
    ...
    if not chart_df.empty and "catalog_chart_eligible" in chart_df.columns:
        verified_chart_df = chart_df[chart_df["catalog_chart_eligible"].fillna(False).astype(bool)].copy()
        if not verified_chart_df.empty:
            chart_df = verified_chart_df
    if chart_df.empty:
    ...
    st.write(
        ...
        f"Effective v16 exponent: **{best['effective_total_price_exponent']:.2f}**. "
    ...
    fig2 = px.bar(
        ...
        title="v16 Desired-Quantity Estimate by Supplier/Product",
    )
    ...
    st.download_button(
        ...
        data=quantity_csv,
        file_name=f"cas_quantity_model_v16_1_{cas_number.replace('-', '_')}.csv",
        mime="text/csv",
    )
...
if run_search:
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"cas_product_evidence_v16_1_{cas_number.replace('-', '_')}.csv",
        mime="text/csv",
    )
    if data_mode == "Live supplier discovery" and "supplier_summary_df" in locals() and not supplier_summary_df.empty:
        ...
        st.download_button(
            ...
            data=summary_csv,
            file_name=f"cas_supplier_summary_v16_1_{cas_number.replace('-', '_')}.csv",
            mime="text/csv",
        )
    ...
    if data_mode == "Live supplier discovery" and "supplier_coverage_df" in locals() and not supplier_coverage_df.empty:
        ...
        st.download_button(
            ...
            data=coverage_csv,
            file_name=f"cas_supplier_coverage_v16_1_{cas_number.replace('-', '_')}.csv",
            mime="text/csv",
        )
else:
    ...
    st.markdown(
        ...
        v16 keeps the baseline intact and adds supplier-specific parser profiles, coverage reporting, exact catalog match logic, and nearest-pack interpolation before any scale-up model.
        """
    )
